(() => {
  ...
  function personCard(person, showImage = true) {
...
function renderPeople() {
  ...
  // Current members → with photo
  $('#currentPeople').innerHTML = data.currentPeople
    .map(person => personCard(person, true))
    .join('');

  // Alumni → without photo
  $('#pastPeople').innerHTML = data.pastPeople
    .map(person => personCard(person, false))
    .join('');

  // visitingPeople → without photo
  $('#visitingPeople').innerHTML = data.visitingPeople
    .map(person => personCard(person, false))
    .join('');
}
          ...
          <article class="publication-card reveal">
            ...
            <div class="publication-content">
              ...
              <p class="publication-authors">
                ${escapeHtml(pub.authors).replace(
                  /Aref Einizade/g,
                  '<strong>Aref Einizade</strong>'
                )}
              </p>
  ...
  function renderTeaching() {
    $('#teachingGrid').innerHTML = data.teaching.map(item => {
      const card = `
        ...
        <h3>${escapeHtml(item.title)}</h3>
        <p>${escapeHtml(item.text)}</p>
        <span class="teaching-cta">View course <span aria-hidden="true">↗</span></span>`;

      return item.href
        ? `<a class="teaching-card teaching-card-link reveal" href="${escapeHtml(item.href)}" aria-label="Open ${escapeHtml(item.title)} course page">${card}</a>`
        : `<article class="teaching-card reveal">${card}</article>`;
    }).join('');
  }
...
function renderNews() {
    ...
    <article class="news-item reveal">
      ...
      ${item.link
        ? `<a href="${escapeHtml(item.link)}"
              ...
              rel="noopener noreferrer"
              class="news-arrow"
              aria-label="Read more: ${escapeHtml(item.title)}">
             ↗
           </a>`
        : `<div class="news-arrow" aria-hidden="true">↗</div>`}
  ...
  function initTriangulation() {
    ...
    const draw = () => {
      ...
      // Slightly stronger graph styling: clearer edges and larger, darker nodes.
      ctx.lineWidth = 1.5;
      tris.forEach(t => {
        ...
        ctx.strokeStyle = hot? 'rgba(14,109,117,.62)': 'rgba(58,88,95,.30)';
        // ctx.strokeStyle = hot ? 'rgba(14,109,117,.48)' : 'rgba(58,88,95,.20)';
        ctx.fillStyle = hot ? 'rgba(14,109,117,.035)' : 'rgba(255,255,255,.006)';
        ctx.fill(); ctx.stroke();
      });
      points.forEach(p => {
        const d=Math.hypot(p.x-mouse.x,p.y-mouse.y);
        // ctx.beginPath(); ctx.arc(p.x,p.y,d<75?3.2:1.8,0,Math.PI*2);
        ctx.beginPath(); ctx.arc(p.x,p.y,d < 75 ? 4.0 : 3,0,Math.PI * 2);
        ctx.fillStyle = d < 75? 'rgba(14,109,117,.95)': 'rgba(38,68,75,.55)'; ctx.fill();
        // ctx.fillStyle = d<75 ? 'rgba(14,109,117,.88)' : 'rgba(38,68,75,.42)'; ctx.fill();
      });
